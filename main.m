...
d = 5;
f = @(x) d*cos(2*x); % f
u = @(x) (delta-(d/12))*cos(4*x) + (gamma/4)*sin(4*x) + (d/12)*cos(2*x); % analytisk lösning!
H = 1/(h^2);
x = linspace(0, 2*pi, N)';
...
for k = 0:4
    ...
    w_history(k+1, 1) = w(end, 1);
end

w_E2 = sqrt(h*abs(u(2*pi)-w_history)); % 2-felet
w_error = u(2*pi) - w_history; % felet

w_ordning = zeros(5, 1);
for j = 2:5
    w_ordning(j) = w_error(j-1)/w_error(j)/2;
end

figure(4)
Ns = [50, 100, 200, 400, 800]';
plot(Ns, w_E2);
title("2-Error of numerical solution given N")
xlabel("N")
ylabel("2-Error")
